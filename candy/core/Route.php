    ...
    */
    ...
    private static function setRoutes($method, $arguments)
    {
        if (in_array($method, self::$allowed_method)) {
            ...
            if (is_array($arguments[1])) {
                if (is_string($arguments[1][0])) {
                    $arguments[1][0] = explode('/', $arguments[1][0]);
                    $arguments[1][0][] = ucfirst(array_pop($arguments[1][0]));
                    $arguments[1][0] = implode('\\', $arguments[1][0]);
                }
            }
            self::$routes[$method][$arguments[0]]['response'] = @$arguments[1];
            self::$routes[$method][$arguments[0]]['parameters'] = (empty($arguments[2])) ? array() : @$arguments[2];
        } else {
    ...
    public function setUri($uri = null, $uri_suffix = '.html')
    {
        ...
        if ($uri == null) {
            ...
            $uri = preg_replace('/'.preg_quote($uri_suffix).'$/is', '', $uri);
            $uri = '/'.implode('/', array_diff_assoc(explode('/', $uri), explode('/', $_SERVER['SCRIPT_NAME'])));
        }
        $this->uri = $uri;
        return $this;
    }
    ...
    {
        if (is_callable($request['response'])) {
            if (is_array($request['response'])) {
                $reflection = new \ReflectionMethod($request['response'][0], $request['response'][1]);
            } else {
                $reflection = new \ReflectionFunction($request['response']);
            }
            $parameters = array();
            if ($args = $reflection->getParameters()) {
                foreach ($args as $arg) {
                    if (isset($request['parameters'][$arg->name])) {
                        $parameters[$arg->name] = $request['parameters'][$arg->name];
                    } else {
                        if ($arg->isDefaultValueAvailable()) {
                            $parameters[$arg->name] = $arg->getDefaultValue();
                        } else {
                            trigger_error('未定义参数：$'.$arg->name, E_USER_WARNING);
                        }
                    }
                }
            }
            call_user_func_array($request['response'], $parameters);
        } elseif (is_string($request['response'])) {
            echo $request['response'];
        } else {
            trigger_error('无效的方法', E_USER_WARNING);
            return false;
        }
    }
